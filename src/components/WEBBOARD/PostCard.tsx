          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 mt-2 bg-gray-900 border border-gray-600 rounded-lg shadow-lg w-32">
                {isOwner && <button onClick={handleEdit} className="block w-full text-left px-4 py-2 hover:bg-gray-700">✏️ แก้ไข</button>}
                <button onClick={handleDelete} className="block w-full text-left px-4 py-2 hover:bg-gray-700 text-red-400">🗑️ ลบ</button>
              </div>
            )}
          </div>
        )}
